...
type SeparatedCodegenStep(tm: TypeManager,
    ...
    override this.Run(km) =
        for k in km.CallGraph.KernelIDs do
            let directives = String.concat "\n\n" (km.CallGraph.GetRequireDirectives(k))
            let structs = km.CallGraph.GetTypesUsage(k)
            let pstructs = String.concat "\n" (List.map (fun (s: Type) -> PrintStructDefinition(s)) structs)
            let functions = String.concat "\n\n" (List.map (fun (f: FunctionInfo) -> f.Codegen) (List.map (fun (m:MethodInfo) -> km.CallGraph.GetFunction(m)) (List.ofSeq(km.CallGraph.GetOutputCalls(k).Keys))))
            let kernel = km.CallGraph.GetKernel(k).Codegen
            let result = String.concat "\n\n" [directives; pstructs; functions; kernel]
            km.CallGraph.GetKernel(k).CustomInfo.Add("SEPARATED_CODE", result)
        (km, "")
