...
module ParameterAccessAnalyzer =
    let Analyze (meth:MethodInfo) =
        ...
        | DerivedPatterns.MethodWithReflectedDefinition(b) ->
            ...
            Array.map (fun (p:ParameterInfo) -> (p, parmsAccess.[p.Name])) (meth.GetParameters())
        | _ ->
            raise (new MetricEvaluationError("Cannon analyze a function not labeled with ReflectedDefinition attribute"))
