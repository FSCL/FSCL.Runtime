...
type ReduceKernelExecutionProcessor() =      
    ...
    member private this.StageReduceCpu((node, isRoot), s, opts) =
        ...
        let localSize = [| 1L |]
        let globalSize = [| node.DeviceData.Device.MaxOpenCLUnits |]
        ...
        // Execute        
        let smallestDataSize = node.KernelData.Kernel.Meta.KernelMeta.Get<MinReduceArrayLengthAttribute>().Length
